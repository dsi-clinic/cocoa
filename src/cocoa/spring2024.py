...
def process_repos(repo_urls):
    ...
    and collecting the results. Each repository is cloned using the `clone_repo` function,
    and then evaluated by calling the `evaluate_repo` function.

    Parameters:
    - repo_urls (list of str): A list of URLs for the Git repositories to be processed.

    Returns:
    - list of tuple: Each tuple contains the repository URL and the result of the evaluation.
